<?php
  ...
  //Generar conexion a BD.
	...
	set @unidad = (Select Usuario.unidad1 from Usuario
		where Usuario.user = 'dirigente1');
	Select Usuario.user from Usuario where Usuario.unidad1 = @unidad and Usuario.tipo = 'niño';
	";
			...
			if($add){
				$query_id_mision = $mysqli->query("SELECT id_mision from mision where nombre_mision = '$nombre_mision'");
					...
					foreach ($users as $key => $value) {
						$add_mision_mapa = $mysqli->query("insert into mision_mapa (user,estado,puntaje,fecha_expiracion,fecha_revision,id_mision) values('$value',0,0,DATE_ADD(CURRENT_DATE, INTERVAL 7 DAY),NULL,$id_mision[0])") or die("Error en los insert".mysql_error());
				}
				echo  json_encode('Mision creada con exito'); // alert msg in react native
			}
			else{
			   echo json_encode('Compruebe su conexión a internet.'); // our query fail
			}
